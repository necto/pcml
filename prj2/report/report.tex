...
Figure \ref{tbl:errClassNN} shows the results. The total BER is $\dfrac{1}{C}\sum Class BER= 11.11\%$ . 
...
We use One-vs-All scheme, because the time limitations are insufficient to train and tweak the One-vs-One SVM. The missprediction rates for each binary classifier are shown in the Table \ref{tbl:SVMerr}. We use HOG features for this method, as it is known to give good results \cite{zhang2010pedestrian}. Unlike the results of Foody&Co. \cite{

Hard-negation mining.

Optimal parameter choice

we choose optimal parameters with manual coordinate descent using cross validation.
...
majority + highest confidence prediction
