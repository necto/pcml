...
The problem may arize for disputed cases (e.g. $y_{horse} == 1$ and $y_{car} == 1$). We may train 3 additional One-to-One classifiers for each of those, if we get enough cases for it.
...
We chose to focus on multi-class classification, and then derive the binary predictions.
...
Finally we combined those three methods to predict the come up with the binary and multi-class predictions we have to submit. 
