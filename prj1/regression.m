...
for k = 1:K
    ...
    beta = leastSquaresGD(yTrTr, tXTrTr, 0.6);
...
tXTr1 = tXTr(yTr<=5500,:);
yTr1 = yTr(yTr<=5500,:);
tXTr2 = tXTr(yTr>5500 & yTr<=10000 & tXTr(:,id2)>0.25 & tXTr(:,id2)<6.4,:);
yTr2 = yTr(yTr>5500 & yTr<=10000 & tXTr(:,id2)>0.25 & tXTr(:,id2)<6.4,:);
...
for i = 1:length(th1)
  for j = 1:length(th2)
    for k = 1:K
        ...
        idxTr2 = find(yTrTr>5500 & yTrTr<=10000 & tXTrTr(:,id2)>0.25 & tXTrTr(:,id2)<6.4);
        idxTe2 = find(yTrTe>5500 & yTrTe<=10000 & tXTrTe(:,id2)>0.25 & tXTrTe(:,id2)<6.4);
